...
| `X9` | **이전 시점에 미종결 항목이 남아 있음** | **WARN (강)** |
| `X10` | `SITETRFDT`가 없어 추출일로 대체됨 | WARN |

`X7`은 사이트 이전 감지입니다. 오타로 인한 잘못된 이전을 잡기 위해 경고로 노출하며, 확정 시 `subject_site_history`에 기록되고 **그 피험자의 전 항목이 새 사이트로 재귀속**됩니다 ([03](03-engine-spec.md) 2.9).

`X9`는 운영 규칙 위반 신호입니다. 이전 전에 모든 pending 항목을 종결하는 것이 전제인데, 남아 있으면 **새 사이트가 이전 사이트의 backlog를 인계받습니다.** 막지는 않되 상단 배너로 강조하고 별도 목록으로 노출합니다.
