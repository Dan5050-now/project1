...
### 1.4 사이트 귀속은 "현재 소속 사이트"로 한다 (검토 반영)

초안은 사이트 이전이 발생하지 않는다고 전제했고, 1차 수정에서는 "활동이 일어난 사이트"에 남기는 방식을 제안했습니다. **검토 결과 두 방식 모두 기각되고, 피험자의 데이터 전체가 새 사이트로 이동**하는 것으로 확정되었습니다.

```
item.site_id = subject.site_id   (현재 소속 사이트)

이전 발생 시  →  그 피험자의 전 항목을 새 사이트로 재귀속
                 분모·분자·backlog 모두 이동
```

#### 운영 규칙이 이 설계를 성립시킨다

이 방식이 타당한 이유는 **이전 전에 기존 사이트에서 모든 pending 항목을 종결한 뒤 이전**하기 때문입니다. 종결된 상태로 넘어가므로 새 사이트가 남의 backlog를 떠안지 않습니다.

| 대안 | 왜 채택하지 않았는가 |
|---|---|
| 활동이 일어난 사이트에 고정 | 한 피험자가 두 사이트에 쪼개져 나타남. 현재 담당 사이트에서 그 피험자를 온전히 볼 수 없음 |
| 비정규화 제거 후 조인 | 롤업 쿼리마다 `subject` 조인. N-1(2초) 달성 곤란 |

#### 운영 규칙이 지켜지지 않으면

앱은 그 규칙을 강제할 수 없습니다. 이전 시점에 미종결 항목이 남아 있으면 **그대로 새 사이트로 이동**하며, 새 사이트가 이전 사이트의 지연을 떠안게 됩니다.

따라서 **이전 감지 시 미종결 항목 수를 점검하고 경고**합니다 (검증 규칙 `X9`). 앱이 막지는 않되, 규칙이 지켜지지 않았다는 사실은 반드시 드러냅니다.

#### 사이트 지표의 의미가 달라진다

전 항목이 이동하므로 사이트별 수치는 **"그 사이트가 수행한 것"이 아니라 "현재 그 사이트가 책임지는 것"** 을 뜻하게 됩니다. 이 차이를 지표 정의 등록부에 명시합니다.

과거 `metric_fact`는 그대로 둡니다. 이전 전 스냅샷은 그 당시 귀속을 유지하므로, **이전을 사이에 둔 두 스냅샷을 비교하면 사이트별 수치가 옮겨 보입니다.** 비교 모드에서 해당 기간에 이전이 있었으면 안내를 표시합니다.
...
| `valid_from` | date | 이 사이트 소속 시작일 = **`DS01.SITETRFDT`** |
| `valid_to` | date | NULL이면 현재 |
| `transfer_date_source` | varchar(20) | `DECLARED`(파일의 이전일) / `INFERRED`(추출일 대체) |
| `open_items_at_transfer` | int | 이전 시점의 미종결 항목 수. 0이 아니면 운영 규칙 위반 |
...
`DS01`에 **`SITETRFDT`(사이트 이전일) 컬럼을 추가**하기로 확정되었습니다 (S-02-5). 이전일이 제공되면 그 날짜가 이력의 기준이 되고, 제공되지 않으면 Drop의 `src_extract_on`으로 대체하되 `transfer_date_source='INFERRED'`로 표시합니다.

귀속 자체는 이전일과 무관합니다(전 항목이 현재 사이트로 가므로). 이전일이 필요한 이유는 **이력의 정확성**과 **미종결 항목 점검 시점**입니다. 점검은 이전일 기준 상태로 수행되어야 하며, 추출일로 대체하면 며칠의 오차가 생깁니다.
...
| `id` | bigserial | |
| `trial_id`, `subject_id` | bigint FK | |
| `site_id`, `country_id` | bigint FK | **그 방문이 수행된 사이트** (`DS02.SITEID`). 사실 기록용이며 **롤업에는 사용하지 않음** |
...
`country_id`와 `site_id`를 비정규화한 것은 롤업 조인을 없애기 위함입니다. 값은 **피험자의 현재 소속 사이트**이며, 이전 시 그 피험자의 **전 항목이 일괄 갱신**됩니다 (1.4).

일괄 갱신은 피험자당 수백~수천 행의 UPDATE이므로 배치로 처리하고, 갱신 건수를 `engine_run`에 기록합니다.
...
| `subject_id`, `site_id`, `country_id` | bigint | 롤업 비정규화. **피험자의 현재 소속 사이트.** 이전 시 함께 갱신 |
...
| ~~S-02-3~~ | ~~피험자 사이트 이전 허용 여부~~ | **확정 — 발생 가능. 전 항목이 새 사이트로 이동** (1.4). 이전 전 종결이 운영 전제이며, 미종결 시 `X9` 경고 |
| S-02-4 | `match_key` 문자열 길이 300이 충분한가 | 샘플 도메인 추가 시 재검토 |
| ~~S-02-5~~ | ~~`DS01`에 사이트 이전일 컬럼 추가~~ | **확정 — `SITETRFDT` 추가** (2.3.1) |
| S-02-7 | 이전 시 일괄 UPDATE의 성능 (피험자당 수천 행) | 배치 처리. 대규모 이전은 야간 창 권고 |
