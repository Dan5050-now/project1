...
| 항목 | 상태 | 왜 여기서 하지 않았는가 |
|---|---|---|
| 결함 E — 공유 워크플로 UI | **✅ 종결** | 파일 창에 `My plans` / `Team plans` 바로가기를 넣고, **개인 폴더에 있는데 남이 저장한 계획서**(손으로 복사해 온 것)에만 한 줄 알림과 **옮기기** 버튼을 붙였습니다. 옮기기는 계획서·저널·이전 판본을 함께 옮기며, 복사→검증→삭제 순이라 중간에 실패해도 원본이 남습니다 |
| 결함 F — 파이썬 설치 관문 | **✅ 종결, 두 겹으로** | 사내 PC에서 파이썬 사용이 **가능함을 확인**했고(2026-09-14), 그 뒤 **파이썬이 없는 PC도 쓸 수 있게** 만들었습니다 — Windows embeddable 런타임을 앱 폴더에 함께 넣는 방식(설치 아님). `PM_APP.cmd` · `tools/bundle_runtime.py` · `check_pc.py` |
| 윈도우 실측 (R-N06) | **대부분 불필요해짐** | 재려던 네 항목 중 셋이 **설계로 제거**되었습니다 — 수정시각 의존은 `last_saved` 토큰으로, `os.access()`는 **탐침 파일**로 대체했고, 이어쓰기는 사람·월별 분리로 완화했습니다. 남은 확인은 `tools/check_share.py` 가 **화면에 12줄**로 답하며, 반출할 파일을 만들지 않습니다 |
