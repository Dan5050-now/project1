...
> **이 문서는 배포 폴더를 직접 고친 단계의 기록입니다 (1/2).** 그 뒤 v1.21의 `src/`를 받아 **소스에 영구 반영**했고, 그 과정에서 요구사항 하나가 절반만 지켜져 있던 것도 찾아 막았습니다. **최신 상태는 `README-v1.21-소스적용.md` 입니다.** 아래 4장 "남은 일"의 첫 줄(`src/` 확보)은 그 문서로 해소되었습니다.
