...
**따라서 결합형 파일에는 양측의 원시값이 모두 들어 있어야 합니다.** 대조 결과만 있고 원시값이 없으면 앱은 자체 판정을 할 수 없고, 그 Feed는 "판정 수입" 모드로 표시되어 **신뢰도 낮음**으로 표기됩니다 (5.6).
...
| DP-12-7 | **사내 external data reconciliation file 샘플 확보** — 4.5·4.9의 컬럼 명세를 실제 구조에 맞춰 조정 | **Phase 0 첫 과제** |
| DP-12-8 | 사내 표준 파일에 **양측 원시값이 들어 있는가**, 아니면 대조 결과만 있는가 | **확인 필수** — 없으면 표준 파일 구조 보완 필요 (3.2) |
