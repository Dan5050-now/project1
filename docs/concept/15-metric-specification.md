...
| 완료 플래그 결측 | `N`으로 간주 |
| 완료 플래그 `Y`인데 완료일 결측 | **완료로 인정**. 단 일관성 경고 ([12](12-standard-source-templates.md) 5.4 X1). 소요일수와 후속 기한은 산출하지 않음 (3.3b) |
| 매칭 키 결측 | 항목 제외 + 검증 거부 |

"기한 산정 불가" 건수를 별도로 집계하는 것이 중요합니다. 조용히 분모에서 빠지면 지표가 이유 없이 좋아 보입니다.

### 3.3b 완료일이 제공되지 않을 때 (R-M1)

vendor가 완료 플래그만 주고 완료일을 주지 않는 경우가 있습니다(예: `BICR_UPLDDTC`, `BA_ANALDTC` 미제공 — [16](16-external-data-format-draft.md) 2.4).

**추정하지 않습니다.** 스냅샷 전환 시점으로 근사하는 방법이 기술적으로 가능하나, 근사값과 실측값이 섞이면 어느 쪽인지 구분되지 않는 날짜가 돌아다니게 되어 지표 전체의 신뢰가 무너집니다. 특히 export된 뒤에는 근사 표시가 유지되지 않습니다.

| 규칙 | 내용 |
|---|---|
| R-M1 | 완료일이 없으면 **완료 여부는 플래그로 판정**한다. 진척률은 정상 산출된다 |
| R-M2 | 해당 단계의 **소요일수(cycle time)는 산출하지 않는다.** 0이나 추정값으로 채우지 않는다 |
| R-M3 | 그 날짜를 트리거로 삼는 **후속 단계는 기한을 계산하지 않는다.** 상태는 `NOT_DUE`이며 건수만 집계한다 |
| R-M4 | 후속 단계의 기한 관리가 불가능하면, 시험 설정에서 그 단계를 **비활성화**하는 것을 우선 검토한다 ([13](13-trial-configuration.md) 4.4) |

R-M4가 중요합니다. 기한 없는 단계를 화면에 남겨두면 `PENDING`만 쌓이는 항목이 되어 오히려 오해를 부릅니다. **관리할 수 없는 단계는 끄는 편이 낫습니다.**
...
| `quality.missing_completion_date` | 완료 플래그는 있으나 완료일이 없는 항목 수 (R-M1) |
...
| E17 | 완료 플래그만 있고 완료일 없음 | 완료로 판정, 소요일수 미산출 (R-M1~M2) |
| E18 | 그 날짜를 트리거로 쓰는 후속 단계 | 기한 미계산 `NOT_DUE`, 건수만 (R-M3) |
