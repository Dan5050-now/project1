...
*v0.4: 근사 완료일 사용 안 함으로 확정 (DP-16-9). 완료일 미제공 시 해당 기한을 계산하지 않음*
...
| `CL_RECVDTC` | 도착 단계 **완료 시점**. 진척률은 `CL_RECVFL`로 가능 | 상태 플래그로 완료 판정, 소요일수와 후속 기한 제외 (2.4) |
...
#### 근사값을 쓰지 않습니다 (DP-16-9 확정)

스냅샷이 불변으로 보존되므로 플래그가 `N`에서 `Y`로 바뀐 시점으로 도착일을 **근사하는 방법이 기술적으로는 가능**합니다. 검토 결과 **사용하지 않기로 확정**했습니다.

근거는 혼선입니다. 근사값은 화면에 표시를 붙이더라도 결국 다른 날짜와 같은 열에 놓이고, export되어 외부 도구로 넘어가면 표시가 떨어져 나갑니다. **근사인지 실측인지 구분되지 않는 날짜가 돌아다니기 시작하면**, 그 값으로 계산된 지연 일수와 기한 판정을 아무도 신뢰하지 않게 됩니다. 이 앱이 지키려는 것이 정확히 그 신뢰이므로, 얻는 것보다 잃는 것이 큽니다.

#### 대신 이렇게 합니다

```
완료 플래그만 있고 완료일이 없는 경우
  ├─ 완료 여부 판정        → 플래그로 수행 (진척률은 정상 산출)
  ├─ 해당 단계의 소요일수   → 산출하지 않음
  └─ 후속 단계의 기한       → 계산하지 않음. 건수만 집계하거나 단계 비활성화
```

즉 **알 수 있는 것만 보여주고, 알 수 없는 것은 만들어내지 않습니다.** 진척률은 그대로 나오므로 손실은 소요일수와 후속 기한에 한정됩니다.

이 결정의 실질적 의미는 **협의에서 완료일 확보의 중요도가 올라간다**는 것입니다. `BICR_UPLDDTC`는 우선순위가 SHOULD이지만, 확보하지 못하면 QC 단계의 기한 관리를 포기해야 하므로 협의에서 우선적으로 요청할 항목입니다. `BA_ANALDTC`도 같습니다.
...
| ~~Q14~~ | ~~스냅샷 기반 근사 도착일을 쓸 것인가~~ | **확정 — 사용하지 않음 (2.4)** |
...
| ~~DP-16-9~~ | ~~스냅샷 기반 근사 도착일 사용 여부~~ | **확정 — 사용하지 않음.** 혼선 위험이 실익보다 큼 (2.4) |
| DP-16-10 | `SRC_EXTRACT_DTC` 확보 수준 — 데이터 최신성 지표의 근거 | 협의에서 확보 권고 |
| DP-16-11 | 완료일 미제공 시 후속 단계를 **비활성화**할 것인가 **건수만 집계**할 것인가 | 단계별 판단. 시험 설정 항목 |
