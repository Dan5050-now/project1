...
> **상세 사양은 [`../spec/`](../spec/README.md)에 있습니다.** 개념과 사양이 충돌하면 개념이 우선하고 사양을 고칩니다.
...
| 값의 추정 | **추정하지 않음.** 알 수 없는 것은 만들어내지 않음 | 추정값과 실측값이 섞이면 구분이 사라지고 지표 전체의 신뢰가 무너짐 |
...
| v2.4 | 2026-09-20 | worksheet 검토 의견 19건 반영 (v0.2). 조건부 우선순위 도입, MUST 미확보 시 대안 정의 |
| v2.5 | 2026-09-20 | **근사 완료일 사용 안 함으로 확정** (DP-16-9). 완료일 미제공 시 완료 여부만 판정하고 소요일수·후속 기한은 산출하지 않음 (R-M1~M4) |
| v2.6 | 2026-09-23 | 사양 검토 반영 — 해결 불가 이슈의 **예외 범위를 이슈 유형별 규칙(`CFG10`)으로** 변경, 피험자 **사이트 이전 허용** (전 항목 이동). `DS01`에 `SITETRFDT` 추가 |
