...
| Step | Description | State |
|---|---|---|
| N1 | Development plan | `docs/PRAP_NewApp_Development_Plan_v0.6.xlsx` — **draft, review round 6** |
...
- `docs/PRAP_NewApp_Development_Plan_v0.6.xlsx` — **current draft, awaiting your review.**
  14 sheets: 63 requirements (NR-ids), the one-engine/two-shells architecture, the
  workspace persistence model and its write protocol, the sharing model on its own
  sheet, 28 engineering decisions (N-01…N-28), 17 risks, a five-gate work breakdown and
  18 questions of which 11 are still open. Generated by `tools/build_newapp_plan.py`.
  It does not supersede or amend anything below. Constraints fixed so far:
  - **Windows 10/11 only.**
...
- `docs/PRAP_NewApp_Development_Plan_v0.5.xlsx` … `_v0.1.xlsx` — superseded.
