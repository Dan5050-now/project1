...
| Step | Description | State |
|---|---|---|
| N1 | Development plan | `docs/PRAP_NewApp_Development_Plan_v0.2.xlsx` — **draft, review round 2** |
...
- `docs/PRAP_NewApp_Development_Plan_v0.2.xlsx` — **current draft, awaiting your review.**
  13 sheets: 38 requirements (NR-ids), the one-engine/two-shells architecture, the
  workspace persistence model and its write protocol, 15 engineering decisions
  (N-01…N-15), 11 risks, a five-gate work breakdown and 13 questions. Generated by
  `tools/build_newapp_plan.py`. It does not supersede or amend anything below.
  Round 1 fixed two constraints: **Windows 10/11 only**, and **non-installed** — a
  folder that is copied and run in place, writing nothing outside itself.
- `docs/PRAP_NewApp_Development_Plan_v0.1.xlsx` — first issue, superseded by v0.2.
