...
| Step | Description | State |
|---|---|---|
| N1 | Development plan | `docs/PRAP_NewApp_Development_Plan_v0.4.xlsx` — **draft, review round 4** |
...
- `docs/PRAP_NewApp_Development_Plan_v0.4.xlsx` — **current draft, awaiting your review.**
  14 sheets: 51 requirements (NR-ids), the one-engine/two-shells architecture, the
  workspace persistence model and its write protocol, the sharing model on its own
  sheet, 22 engineering decisions (N-01…N-22), 15 risks, a five-gate work breakdown and
  17 questions. Generated by `tools/build_newapp_plan.py`. It does not supersede or
  ...
  - **Single writer, many readers** — while one session is editing, no other may update.
    ...
    it is kept alive by a heartbeat so a crashed session frees it by itself; and a
    reader whose figures have been superseded is told, rather than quoting stale ones.
- `docs/PRAP_NewApp_Development_Plan_v0.3.xlsx`, `_v0.2.xlsx`, `_v0.1.xlsx` — superseded.
