...
| Step | Description | State |
|---|---|---|
| N1 | Development plan | `docs/PRAP_NewApp_Development_Plan_v0.3.xlsx` — **draft, review round 3** |
| N2–N5 | Specification, design, build, release | Not started — nothing is built until Gate N1 |
...
- `docs/PRAP_NewApp_Development_Plan_v0.3.xlsx` — **current draft, awaiting your review.**
  13 sheets: 44 requirements (NR-ids), the one-engine/two-shells architecture, the
  workspace persistence model and its write protocol, 18 engineering decisions
  (N-01…N-18), 13 risks, a five-gate work breakdown and 15 questions. Generated by
  `tools/build_newapp_plan.py`. It does not supersede or amend anything below.
  Constraints fixed so far: **Windows 10/11 only**; **non-installed** — a folder copied
  and run in place, writing nothing outside itself and no registry entry of any kind;
  and **shareable** — one copy on a network folder may serve several people, so the data
  location is resolved by a fixed visible rule and a workspace open for editing is
  marked so a second person cannot silently overwrite it.
- `docs/PRAP_NewApp_Development_Plan_v0.2.xlsx`, `_v0.1.xlsx` — superseded.
