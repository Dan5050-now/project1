...
| Step | Description | State |
|---|---|---|
| N1 | Development plan | `docs/PRAP_NewApp_Development_Plan_v0.7.xlsx` — **final draft; all 18 questions answered** |
...
- `docs/PRAP_NewApp_Development_Plan_v0.7.xlsx` — **final draft, ready for Gate N1.**
  The application is **Project Management APP** (`PM_APP`). 14 sheets: 69 requirements
  ...
  (N-01…N-32), 19 risks, a five-gate work breakdown, and all 18 questions answered
  across six review rounds. Generated by `tools/build_newapp_plan.py`. It does not
  supersede or amend anything below. Constraints fixed so far:
  - **Windows 10/11 only.**
  ...
  Two items remain before it becomes the v1.0 baseline: confirming the reading taken of
  the Q-N04 answer (recorded on sheet 12 item 29), and whether the documents should be
  renamed to match the application. Neither blocks Step N2 planning.
- `docs/review/PRAP_NewApp_Development_Plan_v0.6_reviewed.xlsx` — your round-6 answers.
- `docs/PRAP_NewApp_Development_Plan_v0.6.xlsx` … `_v0.1.xlsx` — superseded.
