...
| Step | Description | State |
|---|---|---|
| N1 | Development plan | `docs/PRAP_NewApp_Development_Plan_v0.5.xlsx` — **draft, review round 5** |
...
- `docs/PRAP_NewApp_Development_Plan_v0.5.xlsx` — **current draft, awaiting your review.**
  14 sheets: 61 requirements (NR-ids), the one-engine/two-shells architecture, the
  workspace persistence model and its write protocol, the sharing model on its own
  sheet, 27 engineering decisions (N-01…N-27), 17 risks, a five-gate work breakdown and
  18 questions. Generated by `tools/build_newapp_plan.py`. It does not supersede or
  amend anything below. Constraints fixed so far:
  - **Windows 10/11 only.**
  ...
  - **Named users** — the application asks who you are at launch, so a colleague who
    cannot edit is shown who is holding the plan, how to reach them, and when it frees.
    Declared, not authenticated: it answers "who is editing this", and nothing more.
- `docs/PRAP_NewApp_Development_Plan_v0.4.xlsx` … `_v0.1.xlsx` — superseded.
