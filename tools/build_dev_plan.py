...
rows = [
    ...
    ["1.5", DOC_DATE, "Claude Code", "Pending",
     ...
     "Superseded by v1.6"],
    ["2.40", "2026-09-03", "Claude Code", "Pending",
     "R-32, REQ-CAL-19 AMENDED: A PROJECT-MONTH IS ITS STANDARD, and person_weight now "
     "decides how it is DIVIDED rather than how big it is. The reviewer reported that "
     "the standard was being ignored on import; it was not - it was applied, indexed and "
     "in the arithmetic on every line, and doubling one doubled the figure. What made it "
     "look ignored was person_weight: on the delivered fixture the median project-month "
     "came to 30% of standard x period weight, because the median person weight is 0.29 "
     "and each person's share was scaled by their own weight AFTER the demand was "
     "divided. That was v2.39's chosen reading, taken deliberately so a shortfall would "
     "be visible. Asked again with the arithmetic in front of them, the reviewer chose "
     "the other one. So person_weight, month coverage and the role factor now all make a "
     "CLAIM on the month, and the share is that claim over the sum of the claims: "
     "fte = demand x claim / sum(claims), where demand = standard_fte x period weight x "
     "the month the project actually ran. The shares add to one BY CONSTRUCTION, so the "
     "month is its demand however many people are on it and whatever their weights - "
     "checked on 1,557 and 203 automatic project-months of the two fixtures, all exact. "
     "THE DEMAND IS SCALED BY THE MONTH THE PROJECT RAN, taken as the largest coverage "
     "any of its people have, or every project would spike to a full standard month in "
     "the month it opened and the month it closed. Two consequences worth stating: a "
     "part-time person now pushes load ONTO their colleagues instead of lowering the "
     "project, so under-staffing shows on the PEOPLE as months over the ceiling; and "
     "split_shared_role_fte can no longer inflate a project total, so on a single-role "
     "project it is inert - it still decides the split where roles have different "
     "numbers of holders, which is the case it was always really about. The results "
     "export gains month_run and demand_fte, and every row is now demand x share rather "
     "than a five-term product. Figures move throughout: the 62-project fixture goes "
    ...
    [f"{MARK_NEW}2.60", "2026-09-13", "Claude Code", "Pending",
     ...
     "was in: the note told a planner what an unstaffed project needs, and the screen "
     "still printed a dot for it. "
     "A DOT IS A STATEMENT, AND IT WAS THE WRONG ONE. It says this month costs nothing. "
     "For a month inside the project's own run with nobody on it that is false - it "
     "costs exactly what its standard says - and the falsehood is the same one V-34 "
     "exists for, where a study needing 10.00 and staffed at 5.00 read as a study that "
     "only ever needed 5.00. Here it is staffed at nothing and reads as needing nothing. "
     "DEMAND IS NOW A PROPERTY OF THE PROJECT-MONTH, computed from the PERIODS alone "
     "with no reference to who is on it, by ONE function shared with V-36 - two copies "
     "would eventually disagree, and the symptom would be a screen whose figure "
     "contradicts the finding beside it. "
     "AND IT IS DRAWN AS DEMAND RATHER THAN AS RESOURCE, which is the whole design "
     "problem. The applied figures carry a filled sequential background meaning 'this "
     "much is being done'; none of this is being done, so it takes an outline and a "
     "hatch instead, italic, with a hollow ring glyph so the distinction survives a "
     "monochrome print and a colour-blind reader (D-04). "
     "IT IS NEVER ADDED TO A TOTAL, and that is not fastidiousness. The specification "
     "guarantees that this table's grand total equals the person table's; unallocated "
     "demand belongs to no person, so netting it in would break the one reconciliation "
     "the screen promises AND would assert that somebody is doing the work. It is "
     "totalled on a line of its own and beside each project's applied total - counted "
     "apart and never netted, exactly as short and over are. Measured: applied totals "
     "713.78 against 713.78 with an unstaffed project present, and 38.05 unallocated "
     "beside them. "
     "NOTHING EXISTING MOVES. The fill happens only where the calculation produced no "
     "figure at all; a month that already has one keeps it untouched. That rule is not "
     "cosmetic - the month_run on a line is the greatest COVERAGE any assignment has in "
     "that month, while this is the project's own run, and the two are not always equal, "
     "so recomputing a month that already had a figure would silently move it. On the "
     "delivered fixture the count of unallocated months is ZERO and every figure is "
     "unchanged. "
     "THE HORIZON BUG THAT WAS HIDING BEHIND IT IS FIXED TOO. The span offered by 'show "
     "everything' came only off the assignment lines, so a project starting after every "
     "assignment in the file had ended was not reachable at all - its figures would have "
     "been drawn in months that could not be brought on screen. It is taken from these "
     "months as well now, and the test pushes a project six years out to prove it. "
     "WHAT IS STILL NOT DONE, and is recorded rather than quietly skipped: the demand "
     "charts. A stacked bar that mixed allocated and unallocated bands would misstate "
     "its own total, and the fix is a separate series rather than a taller stack. The "
     "results export is unchanged for the same reason - REQ-OUT-06 promises every "
     "monthly figure is the sum of its detail rows, and unallocated demand has no detail "
     "rows to be the sum of.",
     "Issued for review"],
    [f"{MARK_CHG}2.59", "2026-09-13", "Claude Code", "Pending",
     ...
     "exist. "
     "IT WAS A ROW OF BLANKS, NOT A WRONG NUMBER, WHICH IS WHY NOTHING CAUGHT IT. And "
     "the first wording of this entry got the symptom wrong: it said the project was "
     "absent from everything but the timeline. It is not. SCREENSHOTS SETTLED IT - the "
     "project IS listed in Resource by project, IS drawn on the timeline and IS counted "
     "in the projects tile. What it has is every month EMPTY, nothing on the three "
     "demand charts, no legend entry and no line in Standard vs staffed. That is worse "
     "than being absent, and it is the very failure V-34 exists for: a row of blanks "
     "reads as a project that costs nothing, exactly as a study needing 10.00 and "
     "staffed at 5.00 read as a study that only ever needed 5.00. Type, phase, scope "
     "and periods are all the standard needs; nothing was missing but the asking. "
    ...
    [f"{MARK_NEW}2.53", "2026-09-12", "Claude Code", "Pending",
     ...
     "pressed (D-04). "
     "FIVE. EVERY HEADING SAYS WHAT THE COLUMN MEANS, AND ONLY THAT. The tables used to "
     "show the workbook's own column names: work_scope_type, outsourcing_scope_det, "
     "absorbed_by, ref_id - legible to whoever wrote the schema and opaque to the person "
     "filling the plan in. The meaning was only ever a hover away, which is to say "
     "invisible to anyone who did not know to hover. The heading now prints the plain "
     "name and nothing else. "
     "THE COLUMN'S OWN NAME IS IN THE POP-UP, NOT ON THE PAGE, and this was decided "
     "twice. It was first put on a second line under the label, on the reasoning that "
     "the screen and the file are the same thing, so a heading you cannot find in the "
     "spreadsheet is one you cannot quote in a mail or look up in a document. That "
     "reasoning was right about the NEED and wrong about the PLACE, which the reviewer "
     "said plainly: the need is occasional - you go looking for a column name when you "
     "are editing the workbook or chasing a finding - and the heading is read on every "
     "glance by everyone. Paying for a rare need out of the common one is the wrong way "
     "round when the pop-up already answers it in full. Every heading now carries a "
     "pop-up naming both, and the identifier is on the element as data-cid for whatever "
     "needs to read it. The badge on a derived column names the plain name too - a "
     "heading row that has stopped printing identifiers must not keep one in a badge. "
     "NOTHING IS RENAMED: the file keeps its columns, every validation message still "
     "names them, and the cell still writes back through the identifier. The same head "
     "is used on the heading's pop-up, on every cell's and on the filter buttons', so it "
     "is learned once. "
     "ONE DEFECT EXPOSED, NOT CAUSED, AND FIXED. Widening those three panels put a "
     "table into a gap no layout before it had landed in: the two drawn scroll bars "
     "decide each other, each costing the region 15px of its own space, and paintCue "
     "measured once - so a region that only overflows sideways ONCE ITS OWN VERTICAL "
     "BAR IS DRAWN got no horizontal bar, which is exactly the failure REQ-DSH-13 "
     "exists to prevent. Measured on the 62-project fixture: scrollWidth 1316 with "
     "both flags off and 1318 with the vertical bar there, against a client width of "
     "1316 - two pixels, which is why no earlier layout met it. The flags are now "
     "settled before anything is drawn, and it cannot oscillate because padding can "
     "only create overflow, never remove it. Found by the REQ-DSH-13 check in test_ui, "
     "which failed on this build and passes on the fix. "
    ...
    ["2.51", "2026-09-11", "Claude Code", "Pending",
     ...
     "person whose figure you want to change is very often still on automatic - for whom "
     "the cell rendered as a dash, which reads as 'this screen is read-only for them'. "
     "THE RULE THAT MADE IT A DASH IS STILL THE RULE, and it is why this is not a "
     "one-line change. An automatic assignment ignores MonthlyEstimate entirely, so a "
     "lone row written against it is a figure nothing reads; and setting estimation_type "
     "without seeding the other months counts every one of them as 0.00, the one change "
     "in this application that silently zeroes a figure (REQ-CAL-18). estimation_type "
     "has one way in for exactly that reason, and typing it into a cell is refused "
     "everywhere else. "
     "SO THE CELL ASKS. Type a figure against somebody on automatic and the dialog names "
     "how many months will be stated, what they total, and what this month becomes - "
     "then the switch, the seeding of every month and the typed figure are applied "
     "together, or none of them are. Somebody ALREADY on manual with no figure for that "
     "month is the V-31 case and is NOT asked: the months are already theirs, and asking "
     "would be asking permission for something already given. A figure that is not a "
     "number is refused in the same words the tables use, and nothing is written. "
     "ALL OF IT THROUGH beginEditSession AND S.pending, so the switch, the seed and the "
     "change are three kinds of entry in one change list, undone together by Leave "
     "without change and archived together at Save. The generic cell handler now ignores "
     "a contenteditable cell with no data-sheet, which is what lets a creating cell carry "
     "its own handler instead of calling applyEdit with an undefined sheet and a NaN row. "
     "TWO THINGS WERE CONNECTED WHILE THERE. The Estimation column now reads MANUAL from "
     "the ASSIGNMENT rather than from whether this month happens to carry a row - an "
     "assignment on manual with a month missing was showing as 'auto', which is the "
     "opposite of what is wrong with it. And a row whose Applied is not its Stated now "
    ...
    ["2.50", "2026-09-11", "Claude Code", "Pending",
     ...
     "CANNOT DIFFER - measured in three states before anything was built: projMonth is "
     "built FROM the lines, so it IS that sum, and REQ-OUT-06 rests on it. The pair that "
     "does come apart is DEMAND against APPLIED, and nothing reported it. Measured: a "
     "project needing 10.00 with one assignment cut to 1.00 shows 5.00, people 5.00, "
     "demand 10.00, no finding; the same project set manually to 5.00 gives the same "
     "three numbers and the same silence. A study needing ten people and staffed with "
     "five looked exactly like a study that only ever needed five. "
     "TWO OPTIONS WERE PUT AND NEITHER WAS TAKEN AS PUT. A tab of discrepancies is the "
     "surface you visit only once you already suspect a problem, and the findings report "
     "is already the list of everything wrong; a note in the hover pop-ups is right that "
     "the note belongs where the figure is, but a note you must hover for is nearly as "
     "silent as a tab. So: a RULE, a MARK, a TILE and a SECTION. V-34 rides four "
     "surfaces that already exist. The figure carries a mark in Resource by project and "
     "a dashed outline on the project's own utilisation chart, so nothing has to be "
     "hovered. A tile counts it beside the allocation tiles. And 'Standard vs staffed' "
     "sits on Overall under the tiles, listing every month. "
     "EVERY ROW OPENS THE MONTH AND LETS IT BE FIXED THERE - the requester's own "
     "addition, and the part that makes it a working surface rather than a report. The "
     "stated figures behind the gap are ORDINARY contenteditable cells over "
     "MonthlyEstimate, so the existing editing path validates, logs, marks and undoes "
     "them and there is no second way to change a figure. The dialog redraws itself "
     "after an edit; a panel still showing the gap you had just closed would read as an "
     "edit that did nothing. "
     "BOTH DIRECTIONS, COUNTED APART AND NEVER NETTED. Five short in September against "
     "five over in October sums to zero and is not a plan in balance - the tests hold "
     "that case explicitly. Amber for short and red for over, each with an arrow, "
     "because those two hues are the confusable pair and they are exactly the two "
     "directions (D-04). "
     "IT REPORTS AND NEVER GATES. A warning, so refuses() is false and Save is not held: "
     "departing from the standard is the point of a manual figure, and which figure is "
     "right is not the application's judgement. An all-automatic plan raises nothing, "
     "which is what keeps the rule worth reading. No calculation change and no figure "
     "moves. tools/test_gap.py is new: 32 checks, including that the month equals its "
     "people's sum in every state the fixture can reach.",
     "Superseded by v2.51"],
    ["2.49", "2026-09-06", "Claude Code", "Pending",
    ...
    ["2.48", "2026-09-06", "Claude Code", "Pending",
     ...
     "showed one term of that and left the rest in another tab. "
     "(1) THE PERIODS TABLE now names the STANDARD MONTHLY FTE its period selects, beside "
     "the weight, and multiplies the two: 'Start-up 1.12 | 2.02 -> 2.26 a month'. A row "
     "reading 'x1.12' said this study is an eighth heavier than usual and did not say "
     "heavier than WHAT - answerable only by leaving the tab and reading a 48-row matrix "
     "on General assumptions for one figure. "
     "(2) BOTH MONTHLY ESTIMATION PANELS name the PERIOD the month falls in and its "
     "weight, and (3) THE PERSON'S ALSO NAMES THE DIVISOR - how many people held that "
     "role that month. Those are the two usual explanations for a figure that is not the "
     "size somebody expected: the month moved into a heavier period, or a second holder "
     "arrived and halved the share. The project's panel deliberately does NOT carry the "
     "divisor: a project month is divided between several roles, each with its own count, "
     "so one number there would average things that do not compare. "
     "LOOKED UP, NEVER STORED. None of the three is a column of the sheet it sits beside, "
     "and the tests hold them to that - a standard copied onto a ProjectPeriod row would "
     "survive the standards being edited afterwards, which is the whole failure the "
     "lookup avoids. Read off the calculation's own lines, so a screen cannot name one "
     "period while the figure beside it came from another; calculate() now hands out "
     "periodAt for the one case the lines cannot answer, a manual project month with "
     "nobody assigned to it (V-32). A missing standard says 'none - V-19' rather than "
     "printing the 1.00 the calculation falls back to; that fallback is a degradation and "
     "showing it as a standard would hide exactly what V-19 reports. "
    ...
    ["2.41", "2026-09-04", "Claude Code", "Pending",
     ...
     "FTE-months on the 62-project fixture before and after. "
     "(1) SHEET RENAMED PeriodWeightStandard -> PeriodFTEStandard, and the panel over it "
     "from 'Standard period weights' to 'Standard period FTE for project types'. Schema "
     "10 renamed the COLUMN to standard_fte when it became clear the sheet holds a "
     "magnitude in FTE rather than a multiplier; the sheet and the panel went on saying "
     "'weight', which is the reading that made the standards look ignorable in the first "
     "place - R-32 exists because of it. Source schema version steps 10 to 11. A schema "
     "10 workbook still opens: the old sheet name is translated on the way in and the "
     "reader is told once, exactly as a renamed column is. "
     "(2) THE CALCULATION NOTES on the monthly-estimation panels still described the "
     "pre-R-32 formula - four factors multiplied together - which has not been how a "
     "figure is arrived at since v2.40. They now state it as it is, and separately for "
     "the two levels, because under R-32 the two genuinely differ: a PROJECT month is "
     "standard FTE x period weight x the part of the month the project ran, and an "
     "ASSIGNMENT month is that month divided by share, the share being (role factor / "
     "sharers) x person weight x coverage against everyone else's. Same correction to "
     "the automatic_fte and ProjectPeriod.weight column help and to the switch-back "
     "dialog. "
     "(3) THE WINDOW CHROME OVERLAPPED THE PAGE in the Python shell. Its menu bar and "
     "status strip are sticky at the top of the window and the page's own sticky band - "
     "the edit bar and the four tabs - pinned itself to the same place with a lower "
     "z-index, so scrolling slid the tabs and the Save button underneath the File menu. "
     "The band now sits below the chrome, at the chrome's MEASURED height: both bars "
     "wrap, so a constant would have been wrong exactly when it mattered.",
     "Issued for review"],
    ["2.39", "2026-09-02", "Claude Code", "Pending",
    ...
    ["2.38", "2026-08-30", "Claude Code", "Pending",
     ...
     "rest of it takes, and a standard period weight multiplied by a standard role "
     "factor does not. Project and Assignment each gain estimation_type ('automatic' or "
     "'manual'), and a new sheet MonthlyEstimate carries the stated figures, keyed on "
     "(scope, ref_id, month) so one sheet serves both levels. TWO LEVELS, applied in "
     "that order: an ASSIGNMENT figure IS that person's contribution and replaces the "
     "multiplication outright; a PROJECT figure is the whole month, and the people on it "
     "are SCALED so they still add up to it - chosen over the alternatives because a "
     "project total that did not equal the sum of its people would put the two "
     "utilisation charts in disagreement and cost the results export its one real "
     "guarantee. The scaling factor is recorded on every line, so a person whose figure "
     "moved without anything of their own changing can find out why. MANUAL IS ALL OR "
     "NOTHING for the thing it is set on: switching copies EVERY month across from the "
     "figures currently on screen - to TWO DECIMAL PLACES, because a stated figure is "
     "one somebody reads and edits and 0.01 FTE is already 1.6 hours, so a month can "
     "move by up to 0.005 FTE at the moment of switching - so nothing jumps, and "
     "switching back discards the lot. That was chosen over per-month marking deliberately - once a figure has been "
     "edited, 'which months are still automatic' has no useful answer, because those "
     "would still move under a plan somebody has signed off whenever an assumption "
     "changed. What the user takes on in exchange is responsibility for all of them, and "
     "the confirmation says so in those words. EVERY CHANGE OF CALCULATION WAY IS "
     "CONFIRMED, in both directions, and typing 'manual' into the cell opens the same "
     "dialog rather than setting the flag - the flag without the copy would count every "
     "month as 0.00, which is the one change in the application that could silently zero "
     "a figure. V-31 reports a manual thing with a month carrying no figure; V-32 a "
     "project figure in a month nobody is assigned to, which cannot be shared out and is "
     "therefore NOT applied. Both are raised from the calculation, like V-23, because "
     "they are things that happened to a number. The stated figures round-trip through "
     "the source export, and the results export gains four columns - estimation, "
     "automatic_fte, project_manual_total, project_scale - so a reader can see which "
     "rows were stated, at which level, and what the assumptions would have said "
     "instead. Nothing moves on a file that uses none of it: test_app.py's four-way "
     "comparison agrees on all 1,227 and 441 person-months, and both dummy sets now "
     "carry a manual project and a manual assignment so the rule is under that "
     "comparison rather than in one unit test.",
     "Issued for review"],
    ["2.37", "2026-08-28", "Claude Code", "Pending",
    ...
    ["2.34", "2026-08-27", "Claude Code", "Pending",
     ...
     "typing is what you want on the twentieth row of a set you already know. So the "
     "panel is laid BESIDE the cell, never in front of it: the caret stays put, every "
     "key still reaches the cell, and what is typed steers the grid. Deliberately not "
     "<input type=date>, which would bring one editing model for text cells and another "
     "for dates. (2) R-22: THE PERIOD GENERATOR THAT FITS THE PROJECT. Auto derivation "
     "was offered on 'Others' projects, where it could only ever answer no - the rule "
     "hangs on CTA submission and the DB locks, which an internal project does not have "
     "and never will - and a control that refuses every time it is pressed teaches the "
     "reader the feature is broken. 'Others' is now offered Standard periods, which lays "
     "out Planning / Develop / Close with the dates blank; a trial not ready yet keeps "
     "the derivation, disabled, with the two missing milestones named, because that "
     "answer changes. The derivation itself was checked across all 62 dummy projects and "
     "is correct for all 50 trials. (3) R-23, REQ-DSH-13: THE APPLICATION DRAWS ITS OWN "
     "SCROLLBARS. Twice the browser was asked for a bar and twice it gave an overlay "
     "occupying no layout space - measured, a plain div with an explicit 14px "
     "::-webkit-scrollbar still reports offsetHeight === clientHeight - so a table with "
     "eleven columns off to the right looked like one with none. Native scrolling is "
     "untouched. (4) R-24: THE STICKY BAND IS ONE ELEMENT, AND OPAQUE. Two independently "
     "sticky bars needed the second to know the height of the first, so a hairline ran "
     "between them that the page showed through, and at 92% opacity the rows underneath "
     "ghosted through the text. (5) R-25, REQ-IMP-13: WHAT AN ERROR IS ALLOWED TO DO is "
     "now a property of the rule, in three values. MUST refuses - something is wrong "
     "with the row itself. CONDITIONAL asks at Save, names what will be left unresolved "
     "and lets the user go ahead knowingly. INCOMPLETE reports and says nothing else - "
     "the row is still being built and asking would put a dialog in front of somebody "
     "every time they save a project they are half way through typing. The findings "
     "report and prap_io both label every error with its class.",
     "Issued for review"],
    ["2.33", "2026-08-27", "Claude Code", "Pending",
    ...
    ["2.19", DOC_DATE, "Claude Code", "Pending",
     ...
     "PeriodFTEStandard and RoleFactor grids are NOT embedded: the application builds "
...
sheets_tbl = [
    ...
    [f"{MARK_CHG}PeriodFTEStandard", "THE STANDARD MONTHLY FTE per project type, phase, work scope and period - where every figure gets its size (REQ-CAL-19).", "Reference"],
    ...
    [f"{MARK_NEW}MonthlyEstimate", "Monthly FTE stated by hand instead of calculated, for a project or for one assignment (REQ-CAL-18).", "Child of Project or Assignment"],
...
proj = [
    ...
    [f"{MARK_CHG}work_scope_type", "List", "Yes", "How much of the work is done in-house: 'fully in-housed' / 'fully outsourced' / 'Partially outsourced (in-house for EDC)', extensible on the Lists sheet. SCHEMA 6. Part of the key into PeriodFTEStandard and RoleFactor - a trial run entirely in-house costs this team more than the same trial handed to a CRO, and the weights are where that belongs. Since schema 7 it is the ONLY scope field the calculation reads, and the one the dashboard's scope filter offers.", "R-12, R-16"],
    ...
    [f"{MARK_NEW}estimation_type", "List", "No", "SCHEMA 9. 'automatic' (the default, and what a blank means) or 'manual'. Manual means this project's monthly FTE is STATED on MonthlyEstimate rather than worked out from the assumptions, and the people assigned each month are scaled to add up to the stated figure (REQ-CAL-18).", "REQ-CAL-18"],
...
r = section(ws, r, "Sheet: PeriodFTEStandard")
pws = [
...
rf = [
    ...
    [f"{MARK_NEW}work_scope_type", "List", "No", "The work scope this factor applies to. SCHEMA 6. EMPTY means EVERY scope, exactly as on PeriodFTEStandard - and it matters more here, because this is the larger sheet.", "R-12"],
...
r += 1
r = note(ws, r, "NOTE A CONSEQUENCE. PeriodFTEStandard and RoleFactor now both vary over (project_type, "
                ...
                "Raising a whole project's Conduct load belongs in PeriodFTEStandard, once; doing it in "
...
asg = [
    ...
    [f"{MARK_NEW}estimation_type", "List", "No", "SCHEMA 9. 'automatic' (the default) or 'manual'. Manual means THIS PERSON'S contribution to this project is stated on MonthlyEstimate rather than taken as a share of the project's month - the level below the project's own, and applied first (REQ-CAL-18).", "REQ-CAL-18"],
    [f"{MARK_NEW}estimation_type", "List", "No", "SCHEMA 9. 'automatic' (the default) or 'manual'. Manual means THIS PERSON'S contribution to this project is stated on MonthlyEstimate rather than taken as a share of the project's month - the level below the project's own, and applied first (REQ-CAL-18).", "REQ-CAL-18"],
...
r = section(ws, r, "Sheet: MonthlyEstimate   [new]")
mest = [
    [f"{MARK_NEW}scope", "List", "Yes", "'project' or 'assignment' - which of the two this figure is for. The only sheet whose parent depends on a value IN the row, which is why the key carries it.", "REQ-CAL-18"],
...
r = table(ws, r, ["Column", "Type", "Required", "Definition / rule", "REQ-ID"],
          mest, [26, 11, 12, 88, 14], wrap_cols=(4,), mark_col=1)
r = note(ws, r, "Read only where the owning Project or Assignment carries estimation_type = 'manual'; rows left behind "
                "by a switch back to automatic are ignored rather than obeyed. MANUAL IS ALL OR NOTHING for the thing "
                "it is set on: the application copies EVERY month across when switching, so no month carries a flag of "
                "its own and there is no such thing as a half-manual run. A month with no row after that is a figure "
                "somebody removed, and V-31 says so rather than the application quietly filling it back in.")
...
rules = [
    ...
    [f"{MARK_NEW}V-27", "Every clinical trial's (project_type, clinical_phase, work_scope_type) has at least one row in PeriodFTEStandard - checked on the PROJECT, not on its periods.", "Error - V-19 is precise but only looks at periods a project actually has, so a project whose milestones are missing is never checked at all, and a combination that was never entered is reported once per period rather than once as the one thing that is wrong. This asks the blunter question first: is there any standard for this project at all. Without one, every period would be weighted 1.00."],
...
[f"{MARK_NEW}V-32", "A project set to MANUAL has a figure for a month in which nobody is assigned to it.", "Error - the figure is NOT applied. A project month is shared out among the people on it, so there is nobody to give this to; applying it anyway would give the project a total that none of its people account for, which is exactly the disagreement REQ-CAL-18 scales the people to avoid. Assign somebody to those months, or remove the figure."],
    ...
    [f"{MARK_CHG}V-19", "A clinical trial carries a clinical_phase, and PeriodFTEStandard has rows for that phase. Not applied to 'Others' projects, whose weights are entered directly.", "Error - since Q-26 the phase selects the weights, so a missing phase leaves the project unweighted."],
