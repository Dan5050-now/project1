...
const COLUMN_HELP = {
  ...
  period:"WHICH PERIOD of the project's own plan this month falls in, and the weight that period carries — the row that selected the standard monthly FTE behind the automatic figure. A month belonging to no period is weighted 1.00 by default and V-12 says so. Read off the same calculation the figure came from, so it cannot name a different period from the one the arithmetic used.",
