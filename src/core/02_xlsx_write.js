...
/* SEED-BEGIN — generated by tools/build_app_seed.py from PRAP_SourceData_Template_v1.7.xlsx.
...
const SEED_LISTS = [
  ...
  ["project_type","Biosimilar CT",null],
...
function blankSheets(){
  ...
  for (const t of ct) for (const ph of phases) for (const p of cPeriods)
    sheets.PeriodWeightStandard.push([t, ph, p, 1.00, PLACEHOLDER]);
  for (const t of ct) for (const ph of phases) for (const p of cPeriods) for (const rn of cRoles)
    sheets.RoleFactor.push([t, ph, p, rn, 1.00, PLACEHOLDER]);
  ...
  for (const t of other) for (const p of oPeriods) for (const rn of oRoles)
    sheets.RoleFactor.push([t, null, p, rn, 1.00, PLACEHOLDER]);
  return sheets;
}
